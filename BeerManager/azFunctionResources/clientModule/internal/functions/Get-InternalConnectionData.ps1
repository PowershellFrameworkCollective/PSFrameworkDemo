...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'BeerManager.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'BeerManager.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'BeerManager.Client.ProtectedToken'
