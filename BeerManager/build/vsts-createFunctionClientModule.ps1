...
$parentModule = 'BeerManager'
if (-not $ModuleName) { $ModuleName = 'BeerManager.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\BeerManager'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\BeerManager.psd1"
$prereqHash = @{
